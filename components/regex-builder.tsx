...
function buildRegex(criteria: RegexCriterion[], flags: RegexFlags): string {
    ...
    if (!["starts_with", "ends_with", "exact"].includes(c.type)) {
      switch (c.quantifier) {
        ...
        case "lazy":
          ...
          break
        case "custom":
          if (c.value && c.type !== "custom_class") {
            // custom quantifier is embedded in value like {2,5}
          }
          break
      }
    }
...
export function RegexBuilder({ onSave, editingRegex, onCancelEdit }: RegexBuilderProps) {
  ...
  const handleTest = useCallback(() => {
    ...
    try {
      ...
      if (!patternMatch) {
        setTestResult({ matches: false, matchedParts: [] })
        return
      }
      const re = new RegExp(patternMatch[1], patternMatch[2])
      const allMatches = testString.match(
        new RegExp(patternMatch[1], patternMatch[2].includes("g") ? patternMatch[2] : patternMatch[2] + "g")
      )
      setTestResult({
