...
All errors should be reported to users via toast notifications, never just logged to console. Use the `useToast` hook from `@/components/ui/use-toast`.
...
export function MyComponent() {
  ...
  const handleError = () => {
    toast({
      ...
      description: "User-friendly error message here",
    })
  }
}
```
